...
func (h *Handler) handleMessage(ctx context.Context, evt *events.Message) {
        ...
        // 5) Anti-ban: Add random human-like delay before replying
        // This prevents WhatsApp from detecting bot behavior
        time.Sleep(RandomHumanDelay())

        // 6) Send response with typing presence (best-effort, silent on error).
